...
#unquote() {
#	# same function above, but in bash
#	local _S
#	[ -z "$1" ] && echo -n
#
#	_S="$1"
...
if [ -z "$HTTPD_ROOT" -o ! -z "$HTTPD_CONFFILE" ]; then
	ROOT_LINE=$(egrep -si '^ServerRoot' "$HTTPD_CONFFILE")
	if [ $? -eq 0 -a ! -z "$ROOT_LINE" ]; then
		WORD_CASE=${ROOT_LINE%% *}
		HTTPD_ROOT=${ROOT_LINE#$WORD_CASE}
		HTTPD_ROOT=$(unquote $HTTPD_ROOT)
	fi
	unset ROOT_LINE WORD_CASE
fi
...
#export APACHE_RUN_USER APACHE_RUN_GROUP APACHE_PID_FILE

HTTPD_ENV_FILE="$CONFDIR/envvars"
[ -f "$HTTPD_ENV_FILE" ] || \
	error "envvar file '$HTTPD_ENV_FILE' not found. Ubuntu requires an envvar file to set httpd variables."

source "$HTTPD_ENV_FILE" || \
	error "unable to load httpd env file correctly. Please verify the syntax of the file '$HTTPD_ENV_FILE'"

httpd -t >/dev/null 2>&1 || \
	error "httpd is not in PATH or Apache is unconfigured"

# can mod_dav be loaded?
HTTPD_MODULES_PATH="/usr/lib/apache2/modules/"
httpd -f /dev/null -c "LoadModule dav_module ${HTTPD_MODULES_PATH}mod_dav.so" -t >/dev/null 2>&1 || \
	error "Could not load the mod_dav module, was it installed?"

# can mod_dav_svn be loaded?
httpd -f /dev/null -c "LoadModule dav_module ${HTTPD_MODULES_PATH}mod_dav.so" \
  -c "LoadModule dav_svn_module ${HTTPD_MODULES_PATH}mod_dav_svn.so" -t >/dev/null 2>&1 || \
  error "Could not load the mod_dav_svn module, was it installed?"

# can mod_dav_svn_authz be loaded?
httpd -f /dev/null -c "LoadModule dav_module ${HTTPD_MODULES_PATH}mod_dav.so" \
  -c "LoadModule dav_svn_module ${HTTPD_MODULES_PATH}mod_dav_svn.so" \
  -c "LoadModule authz_svn_module ${HTTPD_MODULES_PATH}mod_authz_svn.so" -t >/dev/null 2>&1 \
...
CONFDIR_INCLUDES="${CONFDIR}/sites-available"
if [ -d "${CONFDIR_INCLUDES}" ]; then
	sed "
s#@@FRONTEND_FQDN@@#$FRONTEND_FQDN#g;
s#@@CLIENTS_MAP@@#$CONFDIR/svn-clients.map#g;
" etc/websvn_frontend.conf > "${CONFDIR_INCLUDES}/websvn_frontend.conf"
	chmod 0600 "${CONFDIR_INCLUDES}/websvn_frontend.conf"
	chown root:root "${CONFDIR_INCLUDES}/websvn_frontend.conf"
...
"
		rm -f -- "${CONFDIR_INCLUDES}/websvn_frontend.conf"
	fi
else
	echo "
WARNING: Could not figure out how to inject a virtual host definition
...
install -p -o root -g root -m755 etc/httpd-svn-ubuntu.init /etc/init.d/httpd-svn


#chkconfig --list httpd-svn >/dev/null 2>&1 || chkconfig --add httpd-svn
#chkconfig httpd-svn on
update-rc.d httpd-svn defaults
