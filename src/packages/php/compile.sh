...
./configure --prefix=/opt/webenabled/bin/packages/php/$version --with-config-file-path=/opt/webenabled/bin/packages/php/$version/etc --with-config-file-scan-dir=/opt/webenabled/bin/packages/php/$version/php.d --with-libdir=lib64 --with-mysq=/usr/lib64 --with-mysqli --enable-fastcgi --enable-force-cgi-redirect --enable-mbstring --enable-pdo=shared  --disable-debug --disable-rpath --with-bz2 --with-curl --with-gettext --with-iconv --with-openssl --with-gd --with-mcrypt --with-pcre-regex --with-zlib --with-ldap --with-xmlrpc
# > /dev/null
#isntall with make
make -j4 > /dev/null
make install  
