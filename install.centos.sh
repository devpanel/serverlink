...
centos_set_variables() {
  _suexec_bin="/usr/sbin/suexec"
  _apache_base_dir="/etc/httpd"
...
centos_install_distro_packages() {
  ...
  yum install php-pear php$php_ver-gd php$php_ver-mysql php$php_ver-mcrypt
  yum install git cgit perl-Digest-HMAC perl-Digest-SHA1
  yum install subversion mod_dav_svn mod_ssl
...
centos_adjust_system_config() {
  ...
  echo 'Include '"$install_dir"'/compat/apache_include/*.conf' \
    >> "$_apache_base_dir"/conf.d/webenabled.conf
  mkdir "$_apache_base_dir"/webenabled.d || error "Cannot mkdir /etc/httpd/webenabled.d: already installed?"
  ...
  mv -f "$_apache_base_dir"/conf.d/php.conf{,.disabled}
